...
- **The number of CarePlans is determined by denominators, not administrative boundaries.** Each CarePlan has exactly one `subject` (denominator) but `targetGeography` is `0..*`. Two configurations are valid: (a) one CarePlan covering several geographies, when those areas are planned and reported as a single scope against one shared denominator (`targetGeography` lists all of them, `subject` is the single regional denominator); (b) several round CarePlans under a regional umbrella linked by `partOf`, {==required whenever each area has its own denominator, period, or coverage rollup==}{>>It is worth seriously thinking through the ramifications of this. A campaign registry will need to track population denominators multiple layers below the district—a common hierarchy might be Region -> State -> District -> Ward -> Health Facility -> Community. The text seems to indicate that each disaggregation of a population estimate is going to require its own `CarePlan` (“(b) several round CarePlans under a regional umbrella linked by partOf, required whenever each area has its own denominator, period, or coverage rollup.”). This would quickly become unwieldly, as districts can have dozens of Wards / Health Facilities and hundreds of communities.<<}{id="93becb2b-4e14-4806-8eec-402265dcf67a" by="James McKinnon" at="2026-07-07T16:59:44.892Z" guest="true"}. Configuration (b) is the common case. The rule is one CarePlan per denominator/reporting scope, not per administrative area.
- **{==Nested scopes do not sum to their parent.==}{>>Just re-iterating my previous comment about allowing diverging population estimates at different levels.<<}{id="403be919-0a12-4d73-914d-bc5d213b43e4" by="James McKinnon" at="2026-07-07T17:00:22.584Z" guest="true"}** A district denominator and the national total are produced by different sources and methods (national 2,150,000 census projection versus Kambia 48,250 WorldPop), so they can legitimately differ. The `partOf` relationship is conceptual nesting, not arithmetic aggregation.
