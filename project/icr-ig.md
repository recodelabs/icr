...
| `quantity` | MS  |     | unsignedInt | {==Group size where individuals are not enumerated (register-level capture) — the fallback; it can coexist with an enumerated `member` list.==}{>>What about the cases where this `quantity` does not agree with the count of enumerated `member`s?<<}{id="c13" by="mckinnoj" at="2026-07-13T10:32:25.674Z"} |
