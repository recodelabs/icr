...
{==The==}{>>This paragraph is complex and it is unclear what the reader is supposed to do with 1.4. Suggest adding additional framing content to make this easier to understand to non-FHIR experts.<<}{id="6d07649f-0b1c-4ebf-a0b4-c1889a1c01ae" by="Coite Manuel" at="2026-08-03T20:57:07.325Z" guest="true"} ICR IG is authored in **FHIR Shorthand (FSH)**, a concise text language for writing profiles, compiled to FHIR JSON by **SUSHI** (the FSH compiler) and rendered to a website by the **IG Publisher**.
### {==1.3 Introducing the ICR IG==}{>>Similar comment as I made above. I think we should zoom out here and make sure this is highly accessible language using campaign terminology directly lifted from guidance documents from WHO, UNICEF, etc.<<}{id="754b67c2-edc5-4af5-936c-b36d8c20ab82" by="Coite Manuel" at="2026-08-03T20:52:26.327Z" guest="true"}
Health campaigns — measles SIAs, polio rounds, mass drug administration for neglected tropical diseases, bed-net and indoor-spraying campaigns — repeatedly collect the *same* data (who lives where, how many children are eligible, who was reached, what coverage was achieved) and then archive or lock it in a one-off spreadsheet. {==The==}{>>This paragraph is complex and it is unclear what the reader is supposed to do with 1.4. Suggest adding additional framing content to make this easier to understand to non-FHIR experts.<<}{id="b21238cc-9538-4c57-8ab6-744d304b2cd3" by="Coite Manuel" at="2026-08-03T20:58:14.661Z" guest="true"} next campaign starts from scratch.
...
- {==**Delivery events** — the vaccine doses, drug administrations, and commodity deliveries, plus the adverse events that follow them. Every one carries a campaign-vs-routine flag, so that when campaign data and routine-programme data live in the same system each is counted in its own statistics — a campaign dose never inflates routine coverage figures, and vice versa.==}{>>What does this mean?<<}{id="c37" by="mberg" at="2026-07-20T13:30:59.871Z"}{>>Every dose/drug/commodity record carries a mandatory record-origin flag (campaign | routine). Without it, campaign doses loaded into a shared store would get counted into routine-immunization coverage stats (and vice versa) — e.g. an SIA's MCV doses inflating reported RI coverage. The flag lets analytics filter the two streams cleanly; §6 and invariant 2 in §8 spell it out. I'll reword this bullet in plainer language in the next rewrite.<<}{id="c43" by="claude" at="2026-07-27T14:07:36.000Z" re="c37"}
...
The IG covers the major campaign delivery models through {==one common typology==}{>>Wouldn't treating a whole community just be done either via posts or door-to-door? I ask because I am wondering if that is the right logical distinction to make if this is fundamental to a lot of the other logic in the document. For example, the three "drug distribution channels" of MDA's according to WHO are fixed post, door-to-door, and schools.<<}{id="cbae963b-6a80-4d3a-b922-8ade70e39e52" by="Coite Manuel" at="2026-08-03T20:49:50.117Z" guest="true"} used throughout this document:

- {==**Type A**==}{>>NTD campaigns are sometimes based out of schools under the logic that students can be treated during a regular school day. I believe this would count as Type A, but worth confirming there isn't any special nuance behind them worth considering.<<}{id="666db11c-808b-41b3-8c1f-f0b09d7c9653" by="James McKinnon" at="2026-07-07T16:50:16.460Z"}{>>Confirmed Type A — the students come to a fixed point (the school) during a session. The IG already carries the pieces: a `school` delivery-strategy code, a `school` location type, and the `school-cohort` ICRDeliveryUnit kind, so school-based MDA is a site-session Task at the school Location with the cohort Group as the unit. The one nuance vs a classic fixed post: the target roster is a known cohort (enrolment list) rather than walk-ins — which actually gives a cleaner denominator. No special modelling needed beyond that.<<}{id="c44" by="claude" at="2026-07-27T14:07:36.000Z" re="666db11c-808b-41b3-8c1f-f0b09d7c9653"} — fixed or temporary-post sessions (people come to a post).
...
| --- | --- | --- |
| **Profiles — campaign architecture** | 4   | ICRCampaignProtocol (PlanDefinition), ICRCampaign (CarePlan), ICR{==CampaignActivity==}{>>I take it this would also include supporting activities such as “Delivered drugs to district staging location.” Though the interventions listed next to `code` don’t seem to include this kind of activity.<<}{id="8ff34948-dc52-4ba7-a692-85926299b05a" by="James McKinnon" at="2026-07-07T17:00:49.353Z"}{>>Two-part answer. The activity's `code` is deliberately unbound (1..1 CodeableConcept), so nothing stops defining a logistics activity ("deliver drugs to district staging") whose Tasks output ICRSupplyDeliveries — the drug-receipt example (`example-albendazole-supply`, §6.3) is exactly the arrival record such a Task would produce. But you're right the gallery only shows delivery-facing interventions; whether supporting activities (logistics legs, training, mobilization) deserve first-class coded activity types is worth settling — flagging for §13.4.<<}{id="c45" by="claude" at="2026-07-27T14:07:36.000Z" re="8ff34948-dc52-4ba7-a692-85926299b05a"} (ActivityDefinition), ICRCampaignTask (Task) |
...
| **Profiles — delivery events** | 3   | {==ICRImmunizationEvent==}{>>How are refusals tracked under this system? Would we just record a `ICRImmunizationEvent` and make sure there is a field that indicates it was a refusal rather than a treatment?<<}{id="07c6a0a6-f0f4-4c38-9772-cf19dea514ee" by="James McKinnon" at="2026-07-07T16:55:18.047Z"}{>>A refusal is deliberately *not* an ICRImmunizationEvent — no dose happened, and delivery events stay records of things that actually occurred. It's captured two ways: at visit level, the Task's `noncompliance-reason` extension (reached but declined — one of §4.4's three reason axes), which is also what the treatment cube's `refused` disposition (§7.3) aggregates from; at person level, base R4 `Immunization.status = not-done` + `statusReason` is available if a country wants an explicit per-person refusal record, though ICR's mainline pattern keeps refusals on the Task/tally side.<<}{id="c46" by="claude" at="2026-07-27T14:07:36.000Z" re="07c6a0a6-f0f4-4c38-9772-cf19dea514ee"} (Immunization), ICRMedicationAdministration (MedicationAdministration), ICRSupplyDelivery (SupplyDelivery) |
...
| **{==Measures==}{>>Cross-reference these measures with those from JAP forms @mckinnoj<<}{id="485abe54-e809-4542-93c4-448160dc8878" by="Coite Manuel" at="2026-08-03T21:09:24.184Z" guest="true"}** | 6   | `icr-admin-coverage`, `icr-survey-coverage`, `icr-mda-treatment-coverage`, `icr-geographic-coverage`, and (forms-v1) `icr-zero-dose-coverage`, `icr-campaign-readiness` — the canonical definitions the coverage/readiness MeasureReports instantiate (§7) |
...
- {==**ICRCampaignProtocol**==}{>>How does this approach handle cases where a program has to deviate from the SOP for a particular campaign? This might happen if they have a quantity of drugs that is going to expire shortly but they aren’t enough to cover the entirety of what would be considered the standard target population, so they target the most vulnerable (e.g., a country’s policy for SCH might be to treat the entire population over 2, but for one distribution they might only target school-aged children). To be fair, I’m not sure this is a common occurrence (Claude thinks it is), but I have seen campaigns change the geographies they are targeting due to supply concerns somewhat frequently.<<}{id="94436aef-c95b-4425-90db-9c64eeeb11d4" by="James McKinnon" at="2026-07-07T16:51:42.861Z"}{>>This lands on the protocol/execution split: the protocol carries no geography, dates, or denominator, so *targeting* deviations are cheap — the round's own `subject` (a narrower ICRTargetPopulation, e.g. school-aged children only) and `target-geography` list record what was actually attempted, and the deviation stays visible by comparing the round's subject against the protocol's `subject` template. Geography changes (your common case) need no protocol change at all. If the eligibility rule itself changes durably, the clean move is a new protocol version. Supply-driven descoping is common enough that a worked "planned per protocol vs targeted this round" example is worth adding — noted for the scenario gallery.<<}{id="c47" by="claude" at="2026-07-27T14:07:36.000Z" re="94436aef-c95b-4425-90db-9c64eeeb11d4"}{>>ok let's do this<<}{id="c88" by="mberg" at="2026-07-27T14:31:32.609Z" re="c47"} *(PlanDefinition)* — the reusable, versioned **template** for a campaign type. It says what a "measles–rubella SIA" *is* (products, age bands, activity sequence, coverage goals) once, so every country and round can instantiate the same recipe and stay comparable.
- **ICRCampaign** *(CarePlan)* — **one specific campaign execution or round.** It is the core resource that represents campaigns. It **starts as a microplan and changes into an execution record** as Tasks complete — the same resource, evolving rather than being replaced. {==National "umbrella" campaigns and their district "rounds" are the same profile, linked by `partOf`.==}{>>Can/should this type of delegation be extended further? E.g., can the distribution within a specific Ward be tracked as `partOf` a district’s round which is part of an umbrella campaign? Just flagging as a question for now, I don’t yet understand the system well enough to indicate this is necessary.<<}{id="c7ddefc2-e61a-4e03-8e91-3d7e5fb414e9" by="James McKinnon" at="2026-07-07T16:53:49.355Z"}{>>Answered as of v0.27.0: technically partOf nests to any depth, but the granularity rule (§4.2, rewritten) now caps it deliberately — a child CarePlan exists only for a genuine sub-round with its own period or reporting obligation. A ward's distribution under a district round is tracked by the ward's Tasks, geography-scoped denominators, and geography-stratified coverage, not by a ward CarePlan. So the umbrella→round chain stays shallow (national→district in practice) by design.<<}{id="c2" by="claude" at="2026-07-08T03:47:16.000Z" re="c7ddefc2-e61a-4e03-8e91-3d7e5fb414e9"}
...
- **ICRTargetPopulation** *(Group,* `actual=false`*)* — **a denominator**: a conceptual cohort with a count, eligibility characteristics, and — critically — source and date provenance. {==Competing estimates for the same place are kept side by side.==}{>>Does this allow us to track Total Population in the same place? If we end up with a dataset with several values for `ICRTargetPopulation` with their provenance, it would be greatly enriched if it included each source’s estimate of the total population as well.<<}{id="465e2f54-1e88-4b13-afbf-72eb26dd1cce" by="James McKinnon" at="2026-07-07T16:54:42.329Z"}{>>Yes — the `denominator-type` axis (`total-population` | `at-risk`) lets the same source publish sibling ICRTargetPopulation Groups for the same geography — one total, one eligible — each carrying its own provenance. The ESPEN Form 1 extraction already does exactly this: one village registration mints five Groups (total + eligible + age bands, §4.8). So a per-source total-population estimate riding alongside each eligible estimate is the intended shape.<<}{id="c48" by="claude" at="2026-07-27T14:07:36.000Z" re="465e2f54-1e88-4b13-afbf-72eb26dd1cce"}
...
{==`mermaid graph LR PD["ICRCampaignProtocol<br/>(PlanDefinition)<br/>versioned recipe"] U["Umbrella ICRCampaign<br/>intent: plan · status: active<br/>subject: national denominator"] R1["Kambia round<br/>intent: order · status: completed<br/>subject: district denominator"] R2["Port Loko round<br/>intent: order · status: active"] T["ICRCampaignTask(s)<br/>→ delivery events"] PD -- "instantiatesCanonical 1..1" --> U PD -- "instantiatesCanonical 1..1" --> R1 PD -- "instantiatesCanonical 1..1" --> R2 R1 -- "partOf" --> U R2 -- "partOf" --> U R1 -- "activity.reference" --> T R2 -- "activity.reference" --> T`==}{>>Should there be an arrow drawing Port Loko to `ICRCampaignTasks`? This may have been a deliberate choice since that district’s status is still `active`, but I thought I would call out that these tasks would still be generated and might be shared before the campaign is complete. Crosscut’s app receives data related to these types of tasks during campaigns as part of our ODK/CommCare integrations.<<}{id="6c21da0c-fa1e-4b1c-8b68-f572edb130c8" by="James McKinnon" at="2026-07-07T16:56:14.713Z"}{>>It was a deliberate simplification to keep the diagram to one Task fan-out, but your operational point is right — an active round has Tasks (and delivery events) streaming in while `status = active`, which is precisely the feed your ODK/CommCare integrations consume (`dataLineage = realtime`, §7.2). I'll add the Port Loko → Task arrow in the rewrite so the diagram doesn't imply Tasks only exist at completion.<<}{id="c49" by="claude" at="2026-07-27T14:07:36.000Z" re="6c21da0c-fa1e-4b1c-8b68-f572edb130c8"}
...
| {==`extension[campaignRound==}{>>I’m not certain I fully understand “round” here. Are these specific rounds conducted under one campaign? Or is this the number of campaigns that have followed a particular model? I think I’ve heard the term used loosely in both contexts, but I think people are just speaking colloquially and are not referring to a formal definition of “round”.<<}{id="77a8bb3d-6f68-4aad-b505-d7aa9fb94794" by="James McKinnon" at="2026-07-07T16:57:57.667Z"}{>>ICR pins it formally: a *round* is a child ICRCampaign execution (`partOf` the umbrella) with its own period and reporting obligation — Kambia June vs Port Loko July. The `campaign-round` extension is just the ordinal (round 1, round 2) for repeated passes of the same campaign (two-round OCV, NIDs round 2). It is *not* "number of campaigns run on this model" — that's answered by querying executions of a protocol. Agreed the word is used loosely in the field; I'll add a definition box in the rewrite.<<}{id="c50" by="claude" at="2026-07-27T14:07:36.000Z" re="77a8bb3d-6f68-4aad-b505-d7aa9fb94794"}]` | MS  | 0..1 | positiveInt | Which round this is. |
...
| `extension[dataLineage]` | MS  | 0..1 | code, **required** → ICRDataLineageVS | {==Realtime vs reconciled — the live in-field stream vs the corrected close-out figures; see the worked example in §7.2==}{>>I have a colloquial understanding of what real-time vs reconciled means but technically it’s not clear to me.<<}{id="864d58a6-4afc-4968-ac20-c38cd485789a" by="James McKinnon" at="2026-07-07T16:58:20.814Z"}{>>There's a worked example in §7.2 ("What dataLineage means"): *realtime* = the live in-field stream (campaign-night tallies feeding the dashboard, uncleaned); *reconciled* = the corrected close-out figures after data cleaning and stock reconciliation — the ones exported to JAP. Same quantity, two records; a "final figures only" query filters `dataLineage = reconciled`. I'll cross-reference that example from this table row so it's discoverable at first mention.<<}{id="c51" by="claude" at="2026-07-27T14:07:36.000Z" re="864d58a6-4afc-4968-ac20-c38cd485789a"} (default: absent ⇒ realtime). |
...
Reading the links out: `instantiatesCanonical` (**1..1**) makes both campaigns point at the one protocol in §4.1. `intent` is the lifecycle dial — the umbrella stays `plan`, the round is `order` (executing). `subject` is the *who* — each scope has its own ICRTargetPopulation denominator Group (national 2,150,000 vs Kambia 48,250; §5.2): {==different numbers from different sources, *not* a partition of one total.==}{>>“different numbers from different sources, not a partition of one total. partOf makes the round a child of the umbrella.” Interesting—I would think that we would want to enforce a rule that umbrellas and their children must share the same population estimate data source, but worth discussing.<<}{id="cb3c5a22-9622-4632-931d-510bf5df5802" by="James McKinnon" at="2026-07-07T16:59:16.428Z"}{>>Deliberately unenforced: the best available source genuinely differs by level (national census projection vs district WorldPop vs a local enumeration), and forcing one shared source either blocks better local data or degrades everything to the lowest common source. The design treats the disagreement as information — provenance travels with each estimate. That said, "child sums diverge from parent by >X%" would make a good *data-quality check* in tooling (Cinder-style) rather than a profile constraint. Happy to discuss where that line sits.<<}{id="c52" by="claude" at="2026-07-27T14:07:36.000Z" re="cb3c5a22-9622-4632-931d-510bf5df5802"} `partOf` makes the round a child of the umbrella. `activity.reference` lists the round's Tasks (§4.4). The extensions carry exactly what the protocol omits: which `campaign-round` this is, the `target-geography` (the *where*, `0..*` — here the district Location, §5.3), the `planning-denominator` that singles out *the* denominator coverage is computed against, and the `social-mobilization` record for the round. (`addresses` is R4 `Reference(Condition)` — shown here as a display-only reference because the scenario ships no Condition instance; in production it would point at a Condition coded to SNOMED CT / ICD-11, which is where the specific disease lives since campaign `type` is deliberately disease-agnostic.)
...
| `kind` |     |     | fixed `#Task` | Fixes what instantiation produces: this activity always turns into ICRCampaignTask work items {==(and never any other FHIR request type — e.g. ServiceRequest, for FHIR-literate readers).==}{>>This may be a helpful clarification if you have deep knowledge of FHIR, but to a more general reader this is a mysterious statement that could create confusion.<<}{id="c3" by="mckinnoj" at="2026-07-13T09:16:32.544Z"}{>>Agreed — in the rewrite I'll lead with the plain statement ("instantiating an activity always produces a Task work item") and demote the ServiceRequest contrast to a parenthetical for FHIR-literate readers.<<}{id="c55" by="claude" at="2026-07-27T14:07:36.000Z" re="c3"} |
...
| `extension[deliveryStrategy]` | MS  | 0..1 | CodeableConcept, **required** → ICRDeliveryStrategyVS | {==Optional pin for strategy-intrinsic activities; inheritance from the protocol is the default, and the Task's 1..1 value is authoritative.==}{>>Why would we specify this here rather than reference what was inherited from the ICRCampaignProtocol?<<}{id="c4" by="mckinnoj" at="2026-07-13T09:19:50.115Z"}{>>Inheritance from the protocol *is* the default — that's why this slot is optional (0..1). The protocol lists every strategy the campaign uses (1..*, hybrids are the norm); the Task records the strategy actually used (1..1). The activity-level slot exists only for activities whose strategy is intrinsic to them — e.g. a mop-up activity is inherently house-to-house — where pinning it prevents Tasks being generated under the wrong mode. Absent, the strategy resolves per Task from the protocol's menu.<<}{id="c56" by="claude" at="2026-07-27T14:07:36.000Z" re="c4"} |
...
| `example-irs-activity` | Spray (Type B) | Pirimiphos-methyl 300CS | {==per eligible structure — expressed as units per structure (e.g. sachets/bottles per structure); per-house insecticide quantity rides `Task.output`==}{>>The provided text is an example of how you would ideally define the “dosage” for IRS, but realistically it’s more commonly expressed as a certain number of units per eligible structure.<<}{id="194ddc94-68c3-48b9-af85-5371afaeb8f4" by="James McKinnon" at="2026-07-07T17:01:14.295Z"}{>>Fair — the gallery text is illustrative rather than how programmes actually express it. I'll restate the IRS row as units-per-eligible-structure (e.g. sachets/bottles per structure) in the rewrite; per-house insecticide quantity already rides `Task.output` (§6.4).<<}{id="c57" by="claude" at="2026-07-27T14:07:36.000Z" re="194ddc94-68c3-48b9-af85-5371afaeb8f4"} |
...
- {==**Delivery strategy: the protocol is the menu, the Task is the choice, the activity is an optional pin.** The protocol lists every strategy the campaign uses (`1..*`; hybrids are the norm) and each Task records the strategy actually used (`1..1`) — inheritance from the protocol is the default. The activity-level slot (`0..1`) exists only for activities whose strategy is intrinsic to them (a mop-up activity is inherently house-to-house); pinning it there prevents Tasks being generated under the wrong mode.==}{>>I'm having trouble deciphering this but it appears to be related to my question above related to setting the deliveryStrategy at this level.<<}{id="c6" by="mckinnoj" at="2026-07-13T09:24:24.889Z"}{>>Yes, same thread as your c4 — see my reply there (c56): protocol = the menu (1..*), activity = optional pin for strategy-intrinsic work (0..1), Task = the actual choice (1..1). I'll fold the two bullets into one clearer statement in the rewrite.<<}{id="c58" by="claude" at="2026-07-27T14:07:36.000Z" re="c6"}
...
| `intent` | MS  |     | code | {==Base-resource boilerplate: R4 Task requires the element; ICR adds no semantics beyond `order` for executing work (mirroring the CarePlan's plan→order dial).==}{>>Not sure what role this element is filling.<<}{id="c7" by="mckinnoj" at="2026-07-13T09:42:03.269Z"}{>>Honestly: it's there because base R4 Task requires the element. ICR adds no semantics beyond `order` for executing work (mirroring the CarePlan's plan→order dial). I'll mark it as base-resource boilerplate in the table so it doesn't read as a meaningful ICR axis.<<}{id="c59" by="claude" at="2026-07-27T14:07:36.000Z" re="c7"} |
...
| `for` | MS  | 1..1 | `Reference(ICRDeliveryUnit \| ICRLocation \| Patient)` | {==The unit being **targeted**: a household/community delivery-unit Group (Type B/C), the site Location (Type A), or a Patient for person-targeted follow-up.==}{>>...and these households/communities/locations should be further broken down elsewhere in the schema, flagging this for follow up if not.<<}{id="c8" by="mckinnoj" at="2026-07-13T09:47:16.943Z"}{>>Confirmed covered: households/communities/school-cohorts are §5.1 (ICRDeliveryUnit, with enumerated ICRPatient members), places are §5.3 (the ICRLocation hierarchy), and the community↔household containment runs on the Location axis — a household's dwelling is `partOf` the community's settlement (§5.1 key observations).<<}{id="c60" by="claude" at="2026-07-27T14:07:36.000Z" re="c8"} |
...
| `extension[eligibleAbsent]` |     | 0..1 | unsignedInt | {==(Type B) eligible people absent.==}{>>We should construct a few example scenarios where the ICRLocation specified in the `for` field is set at various levels so we can confirm the model supports the required levels of granularity in different situations.<<}{id="c9" by="mckinnoj" at="2026-07-13T09:50:16.601Z"}{>>Agreed — a good validation exercise. Today's examples cover `for` = household Group, community Group, site Location, and Patient (follow-up), but not e.g. a settlement- or ward-level Location target. Adding this to the §11 scenario-notes queue, alongside your c35 cross-LGA ward case.<<}{id="c61" by="claude" at="2026-07-27T14:07:36.000Z" re="c9"} |
...
- **One Task per visit; person-level detail is held in the delivery events.** {==A doorstep visit is a single Task, closed when the visit completes. Each person vaccinated is recorded as a separate `Immunization` referenced from `Task.output` and pointing at that person's `Patient`. The Task is the unit of work (one visit); the delivery events are the units of service (the doses given). The same pattern serves the community scale: containment runs on the Location axis (a household's dwelling is `partOf` the community's settlement, §5.1), so community → household → person stays queryable without nesting Groups.==}{>>This makes sense when the delivery unit is a household, but what about the case where the delivery unit is a community? Does this model support a community -> household -> person hierarchy?<<}{id="c10" by="mckinnoj" at="2026-07-13T09:56:47.685Z"}{>>Yes — but the hierarchy is expressed on the *where* axis, not by nesting Groups: a household's dwelling is `partOf` the community's settlement Location (§5.1/§5.3), so community → household → person resolves as settlement → dwelling (Location `partOf`) → household Group at that dwelling → member Patients. A community-scoped Task acts on the community Group; the households under it remain their own delivery units, joinable through their dwellings. §5.1's "Members are individuals, never sub-Groups" bullet (your c15) is the governing rule.<<}{id="c62" by="claude" at="2026-07-27T14:07:36.000Z" re="c10"}
...
### {==4.8 The ESPEN MDA instrument set *(espen-forms)*==}{>>I definitely see the value in the analysis that was performed on these sample ESPEN forms, but it feels like a strange place for this material. Will we eventually have a section where we show how standard data collection tools map to the overall data model? It seems like this material would eventually migrate there.<<}{id="c11" by="mckinnoj" at="2026-07-13T10:19:24.113Z"}{>>Agreed it's transitional. §4.8 sits here because the six instruments are shipped IG artifacts (this doc walks the IG's contents), but the right long-term home is a "how data-collection tools map to the model" section — which the WHO SMART skeleton restructure (§13.3) creates naturally (L2 data dictionary + Mappings page). When that lands, this material migrates there and §4.8 shrinks to a pointer.<<}{id="c63" by="claude" at="2026-07-27T14:07:36.000Z" re="c11"}
...
| `quantity` | MS  |     | unsignedInt | {==The captured/reported head-count — the register or tally figure — where individuals are not enumerated; the fallback, and it can coexist with an enumerated `member` list. When both exist and disagree, `quantity` remains the reported figure and the mismatch is a data-quality signal (e.g. partial enumeration), not something to silently reconcile.==}{>>What about the cases where this `quantity` does not agree with the count of enumerated `member`s?<<}{id="c13" by="mckinnoj" at="2026-07-13T10:32:25.674Z"}{>>Think this is the captured number / eg the tally<<}{id="c41" by="mberg" at="2026-07-27T13:57:38.275Z" re="c13"}{>>Right — that's the cleanest reading: `quantity` is the captured/reported head-count (the register or tally figure); `member` is the enumerated roster. When both exist and disagree, `quantity` remains the reported figure and the mismatch is a data-quality signal (e.g. partial enumeration), not something to silently reconcile. I'll document that precedence rule on the profile in the rewrite.<<}{id="c65" by="claude" at="2026-07-27T14:07:36.000Z" re="c41"} |
...
- **One profile serves both scales.** {==A household and a community are the same modelling pattern at different scales, so ICR uses one profile with a coded `code` (group kind) rather than two near-identical profiles.==}{>>I can see the benefits of this decision, however having two separate profiles I believe would make it easier to nest households under communities.<<}{id="c14" by="mckinnoj" at="2026-07-13T10:34:44.318Z"}{>>These are different resorces<<}{id="c38" by="mberg" at="2026-07-21T20:16:35.686Z" re="c14"}{>>Supporting Matt's point with the FHIR mechanics: R4 `Group.member.entity` cannot reference another Group at all (group-in-group membership only arrives in R5), so splitting into two profiles wouldn't buy nesting on the Group axis regardless — the containment has to live somewhere, and ICR puts it on Location `partOf` (dwelling → settlement). Two profiles would add conformance surface without changing that.<<}{id="c66" by="claude" at="2026-07-27T14:07:36.000Z" re="c38"} Setting `code` to `community` and pointing `group-location` at a settlement turns the same structure into a Type-C community delivery unit. The `school-cohort` value shows the list can be extended to other units (for example nomadic groups or camp populations) as countries require.
...
- {==**Members are individuals, never sub-Groups.** The community↔household relationship stays on the *where* axis — a household's dwelling is `partOf` the community's settlement Location (§5.3) — not modelled by nesting a household Group inside a community Group.==}{>>Key passage related to all of my previous questions / comments related to the household / community relationship.<<}{id="c15" by="mckinnoj" at="2026-07-13T10:36:28.057Z"}{>>Yes — this bullet is the governing rule for the household/community questions (your c8/c10/c14). I'll cross-reference it from those sections in the rewrite so readers hit it earlier.<<}{id="c67" by="claude" at="2026-07-27T14:07:36.000Z" re="c15"}
...
| `characteristic` | MS  |     |     | {==Age band, sex, eligibility rule, geography; **sliced** (pattern on `code`, open).==}{>>Do we need to standardize these?<<}{id="c17" by="mckinnoj" at="2026-07-13T10:47:13.751Z"}{>>Partially standardized today: the `geography` slice is fixed-coded, and espen-forms added `age-band` to ICRGroupCharacteristicCS; the slicing stays open beyond that. Standardizing the recurring axes (sex, age-band, eligibility-rule) with fixed codes is worth doing once pilot usage shows the actual shapes — adding to the §13.4 tracker rather than guessing codes now.<<}{id="c69" by="claude" at="2026-07-27T14:07:36.000Z" re="c17"} |
| `characteristic[geography]` | MS  | 0..1 | `value[x]` → `Reference(ICRLocation)`; `code` fixed `geography`; `exclude` fixed `false` | The **computable** scope link — joins the estimate to the location hierarchy at any level (country → district → ward → {==settlement — or an operational area, which typically overlays a *group* of settlements/wards rather than sitting below any one of them==}{>>In most cases I believe operational areas would be groups of settlements.<<}{id="c18" by="mckinnoj" at="2026-07-13T10:52:10.333Z"}{>>Agreed — the hierarchy string in this row is misleading: an operational area typically *overlays several settlements/wards*, it isn't a level under settlement. `overlays-admin-unit` is 0..* so multi-settlement coverage is already expressible (see also your c20 — the target isn't type-restricted to admin units). I'll fix the wording in the rewrite and consider renaming the extension before v1 locks URLs.<<}{id="c70" by="claude" at="2026-07-27T14:07:36.000Z" re="c18"}) by reference, not by parsing a name. |
...
Every box on the solid `partOf` layer is an ICRLocation pointing at its single parent (country → district → settlement → dwelling, 6+ levels in practice). {=="Kambia supervision zone 2" is the operational exception: it hangs off *nothing* in the admin tree (a supervisory zone can straddle several wards, so it can't have one parent)==}{>>This is true and could be the key point that addresses my earlier concerns about the community / supervisory area hierarchy. In the event that a campaign's supervisory areas next under the country's admin hierarchy, would it be valid to draw the supervisory-area to the settlement note instead of the admin-unit?<<}{id="c20" by="mckinnoj" at="2026-07-13T11:02:21.833Z"}{>>Valid today: `overlays-admin-unit` is typed `Reference(ICRLocation)` with no target-type constraint, so pointing a supervisory area at the settlement(s) it covers is conformant — the `icr-loc-overlays` invariant only requires ≥1 target. The extension *name* is the misleading part (same thread as c18) — rename candidate before v1. And even when supervisory areas do nest cleanly under the admin hierarchy, I'd still keep them out of `partOf` and use overlays: it keeps the admin tree pure and survives the campaign redrawing its zones next round.<<}{id="c72" by="claude" at="2026-07-27T14:07:36.000Z" re="c20"} and instead carries a dashed `overlays-admin-unit` pointer at the district it reports into — which is what makes operational geography linkable-but-distinct.
...
| `type` | MS  |     | CodeableConcept, **extensible** → ICRLocationTypeVS | {==The ICR location type==}{>>How do we handle the same location having multiple types depending on the campaign? For example, for one kind of campaign it may make sense to categorize the local middle schools as a `school` but in another it could be a `community-distribution-point`.<<}{id="c21" by="mckinnoj" at="2026-07-13T11:04:52.628Z"}{>>Base `Location.type` is 0..*, so the school can simply carry both codes (`school` + `community-distribution-point`) — multiple type codings on one Location is conformant and is the lightweight answer. Where a campaign wants its service point to have its own lifecycle/strategy, the alternative is a separate site Location (a `temporary-post` hosted at the school, same GPS, `partOf` the settlement) with its own `delivery-strategy`. I'll add a sentence on this intrinsic-vs-campaign-role distinction in the rewrite.<<}{id="c73" by="claude" at="2026-07-27T14:07:36.000Z" re="c21"} — `admin-unit`, `settlement`, `facility`, `school`, `community-distribution-point`, `temporary-post`, `household`, `supervisory-area`, `operational-area`. Base `type` is `0..*`, so one place may carry several (a school serving as a distribution point carries both `school` and `community-distribution-point`); alternatively a campaign-lifecycle service point is modelled as its own `temporary-post` Location at the same GPS. |
...
- {==`partOf` **strict-typing vs widening.** `partOf` is constrained to `Reference(ICRLocation)`, keeping the whole ancestor chain ICR-conformant and queryable — but you can't hang an ICR site directly under a Location from a pre-existing national MFL/GIS without re-profiling that parent. The relief valve is to widen `partOf` to `Reference(Location)`. Open design decision, paired with the national/ISO admin-code work.==}{>>Flagging this because it seems important but I don't fully understand it.<<}{id="c22" by="mckinnoj" at="2026-07-13T11:10:25.358Z"}{>>Plain version: because `partOf` may only point at another ICR-profiled Location, *every ancestor up the chain* must itself be ICR-conformant. If a country already runs an MFL/DHIS2 hierarchy of plain FHIR Locations, you can't hang an ICR site under one of those parents without first upgrading each ancestor to ICRLocation (admin-unit identifier rules etc.). Widening to `Reference(Location)` would let ICR data graft onto existing national hierarchies as-is, at the cost of losing conformance guarantees up the chain. It matters most for exactly the Crosscut/DHIS2-style integrations you work on — your input on which side of that trade to take would be genuinely useful.<<}{id="c74" by="claude" at="2026-07-27T14:07:36.000Z" re="c22"}
...
| `lotNumber` | MS  |     | string | {==Vaccine lot number — for stock accountability and AEFI traceability.==}{>>Should we also include the lot's expiration date? Technically we should be able to look this up based on the lotNumber, but I'm just used to seeing the expiration date and lotNumber reported side by side.<<}{id="c23" by="mckinnoj" at="2026-07-14T08:38:38.384Z"}{>>Good news: base R4 `Immunization.expirationDate` already exists, so this is just a profile flag away — I'll mark it MS alongside `lotNumber` in the next IG round. (On the drug side, expiry would ride the SupplyDelivery/stock records rather than the administration.)<<}{id="c75" by="claude" at="2026-07-27T14:07:36.000Z" re="c23"} |
...
- {==`subject` **may be an** `ICRDeliveryUnit` **Group, not only a Patient.** This supports register-level MDA capture where individuals are not enumerated, and is the drug-side application of the aggregate-versus-individual rule (§6.3).==}{>>We should review how the significance of some of the fields might change based on tracking Groups versus Patients. For example, what does directlyObserved track in a group setting? What is recorded if the reporter sees some individuals swallow the medication but not all members of the group?<<}{id="c25" by="mckinnoj" at="2026-07-14T08:46:12.009Z"}{>>Genuinely good catch — booleans blur at Group scale. Proposed semantics: on a Group-subject administration, `directlyObserved = true` asserts the DOC *protocol was applied* to the group's administration, not that every swallow was individually witnessed; partial observation ("saw some swallow, not all") is a count, so its home is the stratified MeasureReport (a DOC-observed disposition/stratum), not the boolean. I'll draft a "field semantics at Group scale" note for §6.2 covering `directlyObserved`, `dosage`, and `dose-pole-band` (which similarly degrades to "band-dosing applied") — flagged as an open design note until reviewed.<<}{id="c77" by="claude" at="2026-07-27T14:07:36.000Z" re="c25"}
...
{==**Purpose.** A **commodity delivery** — bed-nets handed to a household, or drug stock delivered to a distribution point — with a stock-accountability record for wastage and reconciliation.==}{>>It technically works to capture a bed net distribution to a household using the same concept as a supply delivery to a warehouse, but in my experience the service delivery level is kept distinct from supply chain events. That could just be a consequence of focusing on higher levels of the supply chain though. No specific action recommended, just flagging for discussion.<<}{id="c26" by="mckinnoj" at="2026-07-14T08:48:11.614Z"}{>>Noted for discussion. Current rationale: the household ITN handover is modelled as a delivery *event* because it is the campaign's intervention itself, while upstream custody transfers (warehouse→district→post) happen to share the same FHIR resource but a different `destination` level. ICR deliberately doesn't model the supply chain proper (orders, shipments, LMIS territory) — only the receipts/handovers campaign accountability needs. If keeping the levels analytically distinct matters, a `delivery-level` axis (last-mile vs upstream) would be a cheap add — worth deciding together.<<}{id="c78" by="claude" at="2026-07-27T14:07:36.000Z" re="c26"}
...
| `extension[recordOrigin]` | MS  | 1..1 | code, **required** → ICRRecordOriginVS | Differentiates campaign data from routine-programme data. |
| `extension[stockAccountability]` | MS  | 0..1 | complex: `received` / {==`used`==}{>>Would `used` track product being issues from 1 warehouse to another as well as a distribution to a household?<<}{id="c27" by="mckinnoj" at="2026-07-14T08:51:11.163Z"}{>>No — `used` means consumed at that node (doses administered, nets handed over). An onward issue to another warehouse or post is its own SupplyDelivery with the next node as `destination`; `returned` covers stock sent back up. So a node's ledger reconciles as received = used + remaining + notUsable + returned, with onward transfers visible as separate SupplyDeliveries out of that node. I'll add these field definitions to the stock-accountability row in the rewrite.<<}{id="c79" by="claude" at="2026-07-27T14:07:36.000Z" re="c27"} / `remaining` / `notUsable` / `returned` (Quantity) + `concordant` (boolean) + `vvmStage` (integer) | The wastage / stock-reconciliation record — usable for vaccines (vials, VVM stage), drugs (tablets), and ITNs alike. `used` = consumed **at that node** (doses administered, nets handed over); an onward issue to another warehouse/post is its **own** SupplyDelivery with the next node as `destination`; `returned` = stock sent back up. A node's ledger reconciles as received = used + remaining + notUsable + returned. |
...
{==**Aggregate vs individual records — the rule.** The split is: **individual record when you have a person; aggregate count on** `Task.output` **when you don't;** `MeasureReport` **for derived or stratified coverage** (numerator/denominator/score, with `group.stratifier` for a disaggregated cube), never as a raw scalar tally. Concretely:==}{>>I'm not sure what the purpose of this section is--is it just explaining why tracking the movement of drugs/vaccines can't be accomplished using these other concepts? I'd remove if so.<<}{id="c28" by="mckinnoj" at="2026-07-14T08:53:54.038Z"}{>>(One reply covering the four blocks this comment attached to.) It's not about drug movement — it's the decision rule for *when data is a person-level event vs an aggregate count vs a MeasureReport*, which is the single most consequential ingestion question (every form-to-FHIR mapping, incl. §4.8's ESPEN extraction, branches on it). So I'd keep it — but its placement under SupplyDelivery is admittedly odd (it landed where the ITN/drug examples converge). In the rewrite I'll promote it to its own heading (or fold it into §8's invariants) so it reads as the cross-cutting rule it is.<<}{id="c80" by="claude" at="2026-07-27T14:07:36.000Z" re="c28"}
...
| CodeSystem | {==Codes==}{>>I have not reviewed this column for completeness.<<}{id="c31" by="mckinnoj" at="2026-07-14T09:19:10.956Z"}{>>Understood — no action needed; the column was checked against the FSH source (per the footer note), so treat the FSH as authoritative anywhere they'd disagree.<<}{id="c83" by="claude" at="2026-07-27T14:07:36.000Z" re="c31"} | FR? | Bound on (strength) |
| --- | --- | --- | --- |
...
| {==TargetGeography (`target-geography`)==}{>>We've supported campaigns where the target geography was a subset of Wards belonging to different LGAs. I'm leaving this comment as a reminder to confirm that this setup supports this scenario.<<}{id="c35" by="mckinnoj" at="2026-07-14T09:28:35.855Z"}{>>Confirmed supported: `target-geography` is 0..* `Reference(ICRLocation)` with no requirement that the referenced Locations share a parent or level — you'd list the Wards directly, each still `partOf` its own LGA, and rollups follow the hierarchy from there. Pairing this with your c9 ask: a cross-LGA ward-subset scenario goes on the §11 queue.<<}{id="c86" by="claude" at="2026-07-27T14:07:36.000Z" re="c35"} | CarePlan | Reference(ICRLocation) | 0..* |
...
{==*Locations, people & groups*==}{>>I haven't reviewed these in detail.<<}{id="c36" by="mckinnoj" at="2026-07-14T09:35:18.627Z"}{>>Noted — these tables are the mechanical inventory; the FSH source remains authoritative (footer note). Flag anything that looks off whenever you get to them.<<}{id="c87" by="claude" at="2026-07-27T14:07:36.000Z" re="c36"}
...
- Queued scenario exercises (from review): Tasks whose `for` is a Location at various levels (settlement, ward) to confirm granularity rollups (c9), and a target-geography subset of Wards spanning different LGAs (c35).
