...
| `subject[x]` | MS  |     |     | The target-population definition (age band, eligibility) — for example "children 9m–14y". |
...
- **Age-band eligibility as computable logic (CQL) is deferred to a later round by design.** This work pairs with the WHO DAK/CQL alignment work (§13.3).
...
**The activity gallery.** Four ActivityDefinitions ship. They span the campaign types. Each one says only WHAT to do, never which concrete target to act on.
...
The shipped Questionnaire puts its items into four sections: **supplies**, **CDD observation**, **stock**, and **social mobilization**. Each section has coded `linkId`s. The example `example-supervision-report` records a CDD observation. In this example, DOC is observed, the height chart is used correctly, and ineligible persons are identified. The stock is not concordant.
...
- **Open design decision.** One question stays open: does a readiness `QuestionnaireResponse` need a dedicated `ICRReadinessReport` profile, parallel to `ICRSupervisionReport`? The alternative reuses the base QuestionnaireResponse pattern. The lifecycle stage is different (pre-campaign, not in-campaign), and this is an argument for a distinct profile. But the structure is identical.
...
- **Campaign-readiness lifecycle** — an `icr-campaign-readiness-checklist` Questionnaire (microplan / cold-chain / social-mobilization / trainings, §4.7) and an `icr-campaign-readiness` roll-up Measure (implementation-unit readiness). *Open: does the readiness QuestionnaireResponse need a dedicated* `ICRReadinessReport` *profile, or does it reuse the supervision pattern?*
...
- **No extraction for the supervision pair, by design.** Per §4.6, the `QuestionnaireResponse` *is* the `ICRSupervisionReport`. Form 4's aggregate side-effect counts also stay on the response. The IG mints no person-level `ICRAdverseEvent` from aggregates.
