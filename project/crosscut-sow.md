...
# Crosscut SOW
<sub>`v0.2.0 · Last modified Jul 21, 2026 at 2:40 PM EDT`</sub>
...
This is the core development phase. Crosscut strengthens its APIs and internal data model to carry ICR concepts, builds the OpenFn-mediated exchange with the ICR FHIR store, improves how ICR data is visualized, and — the majority of the budget — develops new campaign-planning capability: additional catchment approaches with a sharing model, and a new open tool for community identity and de-duplication. Development follows two-week Agile sprints coordinated with Ona.

> [!warning] Prerequisite
> A walkthrough of the Crosscut platform with Ona precedes sprint planning, per the framing above. Scope inside 2.4 and the choice of the two new approaches in 2.5 are set by that walkthrough, not by this document.
...
Technical scope:  
• Extend the internal data model to store concepts Crosscut does not hold today: **campaign events** (doses administered, commodities distributed, coverage results) and **survey/assessment results**, each associable with land blocks, catchment areas, and admin units *(absorbs old 2.3, 2.4)*  
...
• Expose the existing async catchment pipeline as a documented, versioned API surface — job submit, status poll, result retrieval — equivalent to what the frontend already drives *(absorbs old 2.5)*  
• Service-to-service authentication and permission scopes for external callers, extending the existing Cognito + partner-token (JWT) model; security review by Brianna Poulos *(absorbs old 2.11)*  
• Ingest validation: coordinate integrity, hierarchy consistency (no orphaned nodes), polygon validity, duplicate site detection

{>>**Sizing flag.** $5K is thin for what this bucket now carries — the campaign-event and survey storage models alone were priced at $11,955 (old 2.3 + 2.4) and the auth/permission model at $10,361 (old 2.11). Either some of this scope belongs in 2.2 (if the transformation work happens in OpenFn rather than in Crosscut's model), or the bullets above need trimming to fit. My read is that campaign-event storage is the one to interrogate: if Crosscut only ever *reads* campaign data for display, it may not need to store it at all, and 2.1 shrinks to the hierarchy/site/auth work. Worth 15 minutes in the walkthrough.<<}{id="c5" by="claude" at="2026-07-21T18:40:40.000Z"}
...
{>>**ESPEN.** Old 2.9 was a named $7,970 line with Emmanuel Koh as lead; it's now a bullet inside a $10,000 bucket, which under-weights it — the two-way ESPEN flow is one of the higher-value, lower-risk integrations in the whole SOW and it has a specific person attached. If you want it visible to UNICEF as its own deliverable, say so and I'll split it out as 2.2b with its own allocation.<<}{id="c6" by="claude" at="2026-07-21T18:40:40.000Z"}
...
• **Static export of accessibility/isochrone layers** — walking/driving isochrones with configurable time bands — as GeoJSON/FlatGeobuf for consumers that cannot take tiles; today this data is generated but reachable only by authenticated Crosscut frontend users *(absorbs old 2.6)*  
• Distribution via the ICR or direct download; the specific format is chosen against how downstream consumers (DHIS2 Maps, IASO, ODK) will render it, as determined in 1.3
...
• Make ICR-scoped accessibility/travel-time layers visible to the intended audiences (MoH microplanners, UNICEF DAPM) rather than only to Crosscut account holders  
• Support the microplanning workflows documented in 3.2 — the App as an interface for microplanning, to the extent the walkthrough confirms that role

{>>Deliberately the least-specified bucket, because the framing note says the use case for viewing data in Crosscut is exactly what needs discussion. The bullets above are a plausible ceiling, not a commitment. Recommend we treat the walkthrough output as the acceptance criteria for this line and re-write it after.<<}{id="c7" by="claude" at="2026-07-21T18:40:40.000Z"}
...
• **Validation** against known facility service areas using real pilot-country geography and campaign data, feeding corrections back into the algorithms *(absorbs the analytic half of old 2.12)*

{>>Note the workload-balanced candidate above is not invented — it comes straight out of the Crosscut forms analysis, where four malaria workbooks plan IRS against structures-per-operator-day and the MDA forms plan against households-per-CDD-day. See [[crosscut-forms]] §2. If that's the shape of the problem, it argues for workload balancing as one of the two.<<}{id="c8" by="claude" at="2026-07-21T18:40:40.000Z"}
