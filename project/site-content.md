...
| Common model | The ICR defines a shared vocabulary (terminology) for campaigns including campaign types, delivery strategies, missed reasons, team roles, etc. A form built against these codes in ODK, DHIS2, or CommCare produces data the registry understands without a custom mapping. Adopting shared instruments, instead of authoring ones from scratch, makes it easier to compare results across programmes and countries. |
