...
{==**Microplanning produces the campaign's core dataset.**==}{>>I don't like this wording<<}{id="c69" by="mberg" at="2026-06-12T13:37:05.836Z"}{>>Proposed replacement: "**The microplan is a structured dataset, not a document.**" — it states the paragraph's actual point directly and the next sentence already elaborates it ("not a narrative document but a structured dataset"; I'd trim that now-redundant clause too). Alternative if you want to keep the produces-framing: "**Microplanning is where the campaign's dataset is built.**" If your objection was to something else entirely, tell me what — otherwise the first one goes in at the next rewrite.<<}{id="c76" by="claude" at="2026-06-12T19:49:24.000Z" re="c69"} In a well-run campaign the microplan is not a narrative document but a **structured dataset**: every village in the catchment with its target population, hard-to-reach flags, session type (fixed / outreach / mobile), team assignments, transport, and commodity needs — the WHO RED strategy formalized this for routine immunization and campaigns inherited it. The quality payoff is measurable: in Kano State, Nigeria, revised _household-based_ microplanning increased enumerated settlements by 38% and corrected inflated target counts, directly improving the subsequent campaign.
...
* * *
## 6. {==Mapping public-health components → FHIR resources==}{>>One question on this. For the group eg. target population can that be assigned to a settlement, district etc not just a houshold?<<}{id="c70" by="mberg" at="2026-06-12T19:28:19.861Z"}{>>Yes — they're two different Group profiles. ICRHousehold (actual=true) is the enumerated-people case; ICRTargetPopulation (actual=false, a conceptual cohort) is scoped to ANY geography via its `characteristic` — "all children 9mo–14y in District X" or in a settlement, ward, region. The geography lands as a characteristic referencing the Location, at whatever admin level, and ICRCampaign separately carries the target-geography extension → Location. So a district-level campaign never needs household Groups at all. What the doc doesn't yet do is make the geography characteristic explicit in the §7.6 table (it hides inside "geography" in the characteristic row) — I'll spell it out as a named Location-valued characteristic in the next rewrite. Related: your c72 below on community Groups as Task targets.<<}{id="c77" by="claude" at="2026-06-12T19:49:24.000Z" re="c70"}
...
- {==`Encounter`==}{>>When we do the ICR we probably want to call this out.  That We can differentiate with encounters that are part of more routine delivery.<<}{id="c71" by="mberg" at="2026-06-12T19:29:27.430Z"}{>>Agreed — and the differentiation mechanism already exists in the model: `record-origin` (campaign-SIA vs routine-facility-visit, required on every delivery event, §4.4/§7.8) is exactly how a campaign record is told apart from routine delivery, whether or not an Encounter exists. What's missing is saying so HERE: this bullet explains why Task beats Encounter for campaign work but never states the complement — routine delivery keeps its Encounters, and record-origin is the discriminator when both lineages land in the same registry. I'll add that sentence to this bullet in the next rewrite and carry it into the IG's narrative guidance (the ig/ draft already requires record-origin but doesn't explain the Encounter boundary).<<}{id="c78" by="claude" at="2026-06-12T19:49:24.000Z" re="c71"} **for delivery sessions.** Encounter is patient-centric (requires a subject) and carries clinical-visit semantics. Type A site-sessions and Type B household visits are _work_, not visits — `Task` carries assignment, status, location, and outputs natively, and works whether or not a `Patient` exists. Where genuine person-level encounters occur (e.g. EIR-grade capture), `Encounter` remains available _alongside_ the Task, not instead of it.
...
### 7.5 {==ICRHousehold — _profile of_ `Group`==}{>>What about for the things that aren't households eg a settlment or community that's the target for a campaign not a specific household.<<}{id="c72" by="mberg" at="2026-06-12T19:30:48.560Z"}{>>Two cases, both already half-covered but worth making explicit. (1) The target is a PLACE: Task.focus points at the settlement/community Location directly — Type A already works this way (focus = site Location), and a settlement-level Type B Task can do the same. (2) The target is the PEOPLE of a community (the MDA Type C case — §3.2 says "community/household Group" but §7 never defines the community variant): the household pattern generalizes — a Group whose `member`s aren't individually enumerated, `quantity` for size, location extension pointing at the settlement instead of the dwelling. Proposal for the next rewrite: generalize §7.5 from ICRHousehold to the Group+Location pattern with household and community as its two configurations (either one profile with a coded group-kind, or ICRCommunityGroup as a sibling profile — I lean one-profile-with-code, fewer artifacts). Pairs with c70: ICRTargetPopulation handles "everyone in District X" as a denominator; this handles the community as an operational unit a Task acts on.<<}{id="c79" by="claude" at="2026-06-12T19:49:24.000Z" re="c72"}
| Element | Constraint |
|---|---|
| `type` | `person` |
| `actual` | `true` |
...
3. {==**Operational ≠ administrative geography.**==}{>>How is this reflected in the IG?<<}{id="c74" by="mberg" at="2026-06-12T19:41:33.865Z"}{>>Honest answer: only partially, today. The ig/ draft's ICRLocation names operational geography in its description and `type` is open enough to code a supervisory area, but there's no structural mechanism — the location-type codes don't include supervisory-area/operational-area, and `partOf` can only express ONE hierarchy, so an operational area can't both have a parent and declare which admin units it overlays. Fix for the next IG revision: (1) add operational-area / supervisory-area codes to the ICR location-type CodeSystem, and (2) add an `overlays-admin-unit` extension (0..*, Reference(ICRLocation)) linking an operational Location to the admin units it covers — that's what makes it "linkable-but-distinct" rather than just "distinct". I'll note this mechanism here in §9 at the next rewrite and put it on the IG backlog.<<}{id="c81" by="claude" at="2026-06-12T19:49:24.000Z" re="c74"} Polio operational boundaries often differ from RI catchment boundaries (the Nigeria lesson). Model operational geography as _linkable-but-distinct_ from the admin hierarchy.
...
1. {==**Task granularity at scale** — village-level vs household-level Tasks: the performance vs fidelity trade-off for national deployments.==}{>>One issue on tasks.  Do they ahve to be generated in advance or can the be recorded after?  Eg if you know you have to visit 100 households you can generate 100 tasks.  However you may not know how many households are in an area and the job is to register each household as you go.  Those would be just registered as a new task right?  if so, we may want to be able to capture that it was registered in the field.<<}{id="c75" by="mberg" at="2026-06-12T19:42:20.914Z"}{>>Both work natively — nothing in FHIR requires Tasks to pre-exist. Pre-planned: 100 Tasks generated from the microplan at status `requested`, claimed and completed by teams. Discovery mode: the team finds an unenumerated household, the app creates the ICRHousehold (Group + Location) AND its Task on the spot, status going straight to in-progress/completed. And yes to capturing the distinction — it's not just bookkeeping, it's a measurement: the count of field-registered Tasks per area is exactly how incomplete the microplan's enumeration was, which feeds the next round's denominators (§2.3) and the Kano-style microplan-quality story in §1.2. Proposal: a coded `task-origin` element on ICRCampaignTask (pre-planned | field-registered), same required-coded-attribute pattern as record-origin and delivery-strategy. I'll add it to §7.4's extensions and fold the discovery-mode scenario into open question #1 at the next rewrite — it also strengthens the case for household-level Tasks being created lazily rather than pre-generated, which softens the performance side of that question.<<}{id="c82" by="claude" at="2026-06-12T19:49:24.000Z" re="c75"}
