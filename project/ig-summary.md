...
- **ICR geographic-identifier system URIs** *(provisional — to be confirmed before v1.0)*:
  ...
  - `$ISO3166 = urn:iso:std:iso:3166` / `$ISO3166v2 = urn:iso:std:iso:3166:-2` — the **FHIR-designated** system URIs for ISO 3166-1 country and 3166-2 subdivision codes (admin levels 0–1 only; deeper levels have no ISO codes).
  - **National admin codes have no minted URI.** The country's own code — DHIS2 orgUnit UID, MoH code, or the P-code where that *is* the national system — rides the open identifier list under the country's own system URI and is marked `use = official`, giving every consumer one uniform query for *the* admin id: `identifier.where(use = 'official')` (§5.3).
...
| `extension[overlaysAdminUnit]` |     | 0..* | `Reference(ICRLocation)` | For operational geography: the admin unit(s) this area overlays. **Expected when** `type ∈ {supervisory-area, operational-area}` (invariant `icr-loc-overlays`, **warning** in v0.x → error at v1.0). |
...
**Relevant terminology.** `type` binds extensible to **ICRLocationTypeVS** (9 codes incl. `supervisory-area`, `operational-area`). Identifier slices use the geographic-identifier systems from §2.4 (`$GERSId`, `$PCode`, `$ISO3166`, `$ISO3166v2`); the country's own admin code needs no minted URI — it rides the open identifier list under the country's system URI, marked `use = official`.
...
- **Open identifier slicing lets multiple code systems coexist.** National location codes sit alongside GERS, P-codes, and the formal ISO slices without profile changes — under the country's own system URI, marked `use = official` when they are the authoritative code. The `gers` and `pcode` slices are `0..1`, so a newly created, unmatched location can exist with national codes only and have its GERS ID back-filled later. The enrichment lifecycle is: create the location unmatched, run asynchronous conflation, then backfill the GERS ID with versioning and Provenance.
- **Administrative units must carry an identifier — any identifier.** The `icr-loc-admin-id` invariant (**error**) requires at least one identifier from any system when `type = admin-unit`, so an administrative area cannot exist without a stable code; the companion `icr-loc-admin-official` invariant (**warning**, promoted to error at v1.0) expects the country's authoritative code to be marked `use = official`, giving consumers one uniform query for *the* admin id regardless of country system. Sites and dwellings are not constrained this way.{>>Confirmed and fixed — your probes were right: neither invariant existed in the FSH (the doc was ahead of the IG). Decisions applied today: icr-loc-admin-id now ships at ERROR severity, so probe-location-admin-no-identifier fails validation, plus a new icr-loc-admin-official (warning) expecting the country's authoritative code to be marked use = official. The `national` slice and the $NationalAdminCode placeholder URI are dropped — there is no honest single URI for per-country code systems (usually a DHIS2 orgUnit namespace), so national codes ride the open slicing under the country's own URI with the official mark; formal iso slices were added on FHIR's standard 3166 URNs instead.<<}{id="c15" by="claude" at="2026-08-10T19:58:29.000Z"}
- **Operational geography is modelled separately from administrative geography.** `partOf` can express only one hierarchy. A supervisory or operational area is therefore typed with a location-type code and linked to the administrative units it covers through `overlays-admin-unit`. The `icr-loc-overlays` invariant expects such an area to overlay at least one admin unit (**warning** in v0.x, promoted to error at v1.0), because an area that overlays nothing cannot be rolled up to any reporting unit.{>>Confirmed and fixed — the invariant did not exist; it now ships at WARNING severity (Matt's call today: outbreak-response areas may be drawn before admin mapping), promoted to error at v1.0. Your probe-location-supervisory-no-overlay now surfaces a validator warning and will hard-fail after the promotion.<<}{id="c16" by="claude" at="2026-08-10T19:58:29.000Z"} This operational-overlay mechanism is considered the IG's strongest design feature.
...
| OverlaysAdminUnit (`overlays-admin-unit`) | Location | Reference(ICRLocation) — links operational geography to the admin unit(s) it overlays; expected on supervisory/operational-area types (invariant `icr-loc-overlays`, warning → error at v1.0) |
