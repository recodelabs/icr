...
| **3. Treatment tally** | `fromTreatmentForm` | The campaign anchor (`ICRCampaignProtocol` per state × year → `ICRCampaign` **district** round with the district eligible population as its global target → one completed `ICRCampaignTask` per submission), an `ICRDeliveryUnit` community Group, one Group-subject `ICRMedicationAdministration` per treated drug (the treatment event), and an `ICRAdministrativeCoverage` (MeasureReport) per drug — numerator/denominator + `sex` × `age-band` × `disposition` stratifiers |
