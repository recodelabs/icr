#!/usr/bin/env bash
# Build ODK Locations and publish it to https://odklocations.healthcampaigns.org.
#
#   observable build ──▶ dist/ ──rclone sync──▶ r2:icr/_site/odklocations/ ──▶ wrangler deploy (Worker)
#
...
echo "== sync dist/ → $REMOTE:$BUCKET/_site/odklocations ${DRY:+(dry run)}"
rclone sync $DRY --checksum --fast-list --transfers 16 --exclude ".DS_Store" \
  dist "$REMOTE:$BUCKET/_site/odklocations" --stats-one-line -v 2>&1 | grep -v "^$" | tail -3

if [ -z "$DRY" ]; then
  echo "== deploy worker"
  npx wrangler deploy
  echo "== https://odklocations.healthcampaigns.org"
fi
