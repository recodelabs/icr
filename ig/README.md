...
- `input/fsh/` — all FSH source:
  ...
  - `searchparameters.fsh` — custom SearchParameters: `CarePlan?target-geography=` and `Group?geography=` (campaign visibility: what is planned where; see `tools/hapi/README.md` for the query recipes); `Location?quadkey=` (prefix = tile containment) and `Location?h3=` (spatial-index cells)
