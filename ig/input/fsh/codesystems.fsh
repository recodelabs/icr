...
* #facility-catchment "Facility catchment" "The service (catchment) area of one health facility — the polygon of territory whose population that facility serves, e.g. a ward-bounded Voronoi or travel-time catchment. A separate Location from the facility itself: physicalType area, boundary polygon, catchment-of → the facility Location, partOf the lowest admin unit that fully contains it. Population denominators, access and distance attributes for the facility's service area are scoped to this Location, not to the facility site."
* #settlement-catchment "Settlement catchment" "The area (built-up footprint or catchment) of one settlement, bounded by and partOf the facility catchment it is assigned to. A separate Location from the settlement point: physicalType area, boundary polygon, catchment-of → the settlement Location. Carries the settlement-level population and accessibility attributes."
...
* #travel-time "Travel time to service point" "The travel-time band to the nearest (or the catchment's own) health facility this estimate is scoped to — an accessibility stratum of the geography's population (ICRTravelTimeBandVS: walking / driving, under 1 h, 1–4 h, over 4 h). Pairs with the geography characteristic: 'people in this catchment more than 4 hours' walk from the facility' (catchments round, Sep 2026)."

CodeSystem: ICRTravelTimeBandCS
Id: icr-travel-time-band-cs
Title: "ICR Travel Time Band"
Description: "Accessibility strata for a population estimate: travel time to the service point (the catchment's facility, or the nearest facility), by mode. Computed on a friction surface (e.g. Crosscut / MAP accessibility models over WorldPop); the band edges follow the common 1 h / 4 h cut-offs."
* ^caseSensitive = true
* ^experimental = false
* #walk-lt-1h "Within 1 hour walking" "Less than one hour on foot."
* #walk-1h-4h "1 to 4 hours walking" "Between one and four hours on foot."
* #walk-gt-4h "Over 4 hours walking" "More than four hours on foot."
* #drive-lt-1h "Within 1 hour driving" "Less than one hour by motor vehicle."
* #drive-1h-4h "1 to 4 hours driving" "Between one and four hours by motor vehicle."
* #drive-gt-4h "Over 4 hours driving" "More than four hours by motor vehicle."

CodeSystem: ICRBuildingSourceCS
Id: icr-building-source-cs
Title: "ICR Building Footprint Source"
Description: "The building-footprint dataset behind a building count on a Location (building-count extension). Counts from different datasets are not comparable — each dataset has its own date, detection method and confidence threshold — so every count names its source."
* ^caseSensitive = true
* ^experimental = false
* #osm "OpenStreetMap" "OpenStreetMap building footprints (community-mapped)."
* #overture "Overture Maps" "Overture Maps Foundation buildings (conflated OSM, Microsoft, Google and Esri footprints)."
* #google-open-buildings "Google Open Buildings" "Google Open Buildings footprints (satellite-detected); record the confidence threshold used."
* #microsoft "Microsoft Building Footprints" "Microsoft global ML building footprints."
...
Description: "Provenance tags applied as meta.tag on the instances that ship with this IG (worked examples and demo datasets), so they can be filtered or excluded in one search (?_tag=). This is NOT a normative vocabulary: implementers tag their own datasets with their own code system (tags are not bound to any value set); the codes here exist only so the IG's own instances validate cleanly."
* ^caseSensitive = true
* ^experimental = false
...
* #nga-demo "Nigeria demo calendar" "The five-state northern Nigeria demo campaign calendar (Bauchi, Kano, Jigawa, Gombe, Yobe; 2022–2027) generated by tools/campaign-builder — CarePlans, protocols and planning denominators only."
* #nga-microplan-demo "Nigeria microplan demo" "The Toro LGA (Bauchi) polio NIPDs microplanning + monitoring snapshot generated by tools/microplan-builder: one facility-level ICRCampaign per health-facility catchment under an LGA round, a care team per facility, under-5 denominators per settlement catchment, and one house-to-house ICRCampaignTask per settlement with tally outputs as of a snapshot date."
